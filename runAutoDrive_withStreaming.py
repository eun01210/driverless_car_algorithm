...
class VideoStreaming:
    ...
    def imageProcessing(self, image, LiDAR, num, line):
        ...
        try:
            ...
            # get drive command
            Save_Cr_image = cv2.addWeighted(undist_image, 0.8, canny_img, 1, 1)
 
            self.command, self.status, self.light = autoDrive_algorithm(undist_image, canny_img, gray_img, points, LiDAR, self.command, self.status, self.light)
            
            Save_Cr_image = cv2.putText(Save_Cr_image, "%s" % self.command, (0,15), cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 255, 255))
            Save_Cr_image = cv2.putText(Save_Cr_image, "%s, %s, %s" % (line[0], line[1], line[2]), (0,30), cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 255, 255))
            cv2.imwrite('saved_image\%d_image.jpg' % num, Save_Cr_image)
            print(num)
            num = num + 1
            
            cv2.imshow('saved_image', Save_Cr_image)
